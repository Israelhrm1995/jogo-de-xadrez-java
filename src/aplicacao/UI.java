...
public class UI {
	...
	public static XadrezPosicao lerXadrezPosicao(Scanner sc) {
		...
		return new XadrezPosicao(coluna, (char) linha);
		}
		catch (RuntimeException e) {
			throw new InputMismatchException("Erro ao ler uma posicao no xadrez os valores permitidos sao de a1 a h8");
		}
	}
	...
	private static void imprimirPecasCapturadas(List<XadrezPeca> capturadas) {
		List<XadrezPeca> branco = capturadas.stream().filter(x -> x.getCor() == Cor.BRANCO).collect(Collectors.toList());
